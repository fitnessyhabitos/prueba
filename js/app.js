...
console.log("⚡ FIT DATA: App v15.0 (FINAL STABLE - Persistent)...");

// --- 1. REGISTRO SERVICE WORKER (OFFLINE MODE) ---
...
// Persistencia de datos (Defensive Programming)
try { 
...
} catch(e) { console.debug("Persistencia ya activa o no disponible."); }
...
// Variables Infinite Scroll (Optimización)
...
onAuthStateChanged(auth, async (user) => {
    ...
    if(user) {
        ...
        // 1. Restauración INMEDIATA desde LocalStorage (Persistencia)
        const savedW = localStorage.getItem('fit_active_workout');
        if(savedW) { 
            try { 
                activeWorkout = JSON.parse(savedW);
                // Si hay entreno, iniciamos el timer en segundo plano para la barra flotante
                startTimerMini();
        ...
        try {
            ...
            if(snap.exists()){
                ...
                if(userData.approved){
                    ...
                    // Si tenemos entreno activo restaurado, vamos allí directamente
                    if(activeWorkout) {
                        renderWorkout();
...
window.switchTab = (t) => {
    ...
    if (t === 'workout-view') {
        // Seguridad: Si no hay entreno, volvemos a rutinas
...
// FUNCIÓN DE BORRADO DE HISTORIAL
window.deleteHistoryWorkout = async (id) => {
    ...
    try {
        ...
        alert("✅ Entreno eliminado. Tus estadísticas históricas se corregirán.");
        window.loadProfile();
...
window.startWorkout = async (rid) => {
    ...
    try {
        ...
        // Lógica corregida para buscar el último entreno válido (ignorando los borrados)
...
window.finishWorkout = async (rpeVal) => { try { window.closeModal('modal-rpe'); const note = document.getElementById('workout-notes')?.value || ""; let totalSets = 0, totalReps = 0, totalKg = 0; let muscleCounts = {}; let missingWeights = false; const cleanLog = activeWorkout.exs.map(e => { const completedSets = e.sets.filter(set => set.d).map(set => { const r = parseInt(set.r) || 0; const w = parseFloat(set.w) || 0; if (w === 0) missingWeights = true; totalSets++; totalReps += r; totalKg += (r * w); const mName = e.mInfo?.main || "General"; muscleCounts[mName] = (muscleCounts[mName] || 0) + 1; return { r, w, isDrop: !!set.isDrop, numDisplay: String(set.numDisplay || "") }; }); return { n: e.n, s: completedSets, superset: !!e.superset, note: e.note || "" }; }).filter(e => e.s.length > 0); if (cleanLog.length === 0) { alert("No hay series completadas."); return; } if(missingWeights) { if(!confirm("⚠️ ATENCIÓN:\n\nHay series marcadas como hechas pero con 0 kg.\n\n¿Estás seguro de que quieres terminar el entreno así?")) return; } const startTime = activeWorkout.startTime || Date.now(); const durationMs = Date.now() - startTime; const h = Math.floor(durationMs / 3600000); const m = Math.floor((durationMs % 3600000) / 60000); const s = Math.floor((durationMs % 60000) / 1000); const durationStr = h > 0 ? `${h}h ${m}m ${s}s` : `${m}m ${s}s`; const workoutNum = (userData.stats?.workouts || 0) + 1; const now = new Date(); const currentMonthKey = `${now.getFullYear()}_${now.getMonth()}`; const currentYearKey = `${now.getFullYear()}`; const currentWeekKey = getWeekNumber(now); await addDoc(collection(db, "workouts"), { uid: currentUser.uid, date: serverTimestamp(), routine: activeWorkout.name || "Rutina sin nombre", rpe: rpeVal, note: note, details: cleanLog, workoutNumber: workoutNum, sessionVolume: Number(totalKg.toFixed(2)), duration: durationStr, monthKey: currentMonthKey, yearKey: currentYearKey, weekKey: currentWeekKey }); const updates = { "stats.workouts": increment(1), "stats.totalSets": increment(totalSets), "stats.totalReps": increment(totalReps), "stats.totalKg": increment(totalKg), "prs": userData.prs || {}, "lastWorkoutDate": serverTimestamp() }; updates[`stats_week_${currentWeekKey}.kg`] = increment(totalKg); updates[`stats_week_${currentWeekKey}.workouts`] = increment(1); updates[`stats_week_${currentWeekKey}.reps`] = increment(totalReps); updates[`stats_month_${currentMonthKey}.kg`] = increment(totalKg); updates[`stats_month_${currentMonthKey}.workouts`] = increment(1); updates[`stats_month_${currentMonthKey}.reps`] = increment(totalReps); updates[`stats_month_${currentMonthKey}.sets`] = increment(totalSets); updates[`stats_year_${currentYearKey}.kg`] = increment(totalKg); updates[`stats_year_${currentYearKey}.workouts`] = increment(1); updates[`stats_year_${currentYearKey}.reps`] = increment(totalReps); for (const [muscle, count] of Object.entries(muscleCounts)) { updates[`muscleStats.${muscle}`] = increment(count); } await updateDoc(doc(db, "users", currentUser.uid), updates); compressAndCleanupWorkouts(currentUser.uid); showToast(`🏆 ¡Entreno nº ${workoutNum} completado! Tiempo: ${durationStr}`); localStorage.removeItem('fit_active_workout'); activeWorkout = null; document.getElementById('active-workout-bar').classList.add('hidden'); if (durationInt) clearInterval(durationInt); if (wakeLock) { await wakeLock.release(); wakeLock = null; } window.switchTab('routines-view'); } catch (error) { console.error("Error finish:", error); alert("Error crítico al guardar. Revisa tu conexión."); } };
