...
console.log("⚡ FIT DATA: App v13.0 (Performance & Offline Mode)...");

/* ===========================================================
   1. SERVICE WORKER REGISTRATION (Mejora 2: Instant Load)
   =========================================================== */
...
// Persistencia defensiva para Offline
try {
...
// Variables para Infinite Scroll (Mejora 3)
...
onAuthStateChanged(auth, async (user) => {
    // Lazy Load UI: Ocultamos loading screen lo antes posible
        ...
        try {
            ...
            if(snap.exists()){
                ...
                if(userData.approved){
                    ...
                    // Recuperar sesión activa (Mejora 4)
                    const savedW = localStorage.getItem('fit_active_workout');
                    if(savedW) { 
                        try {
                            activeWorkout = JSON.parse(savedW); 
                            // No cambiamos de pestaña automáticamente, dejamos que el usuario decida,
                            // pero mostramos indicador visual en el botón de Routines
                            renderWorkout(); 
                            startTimerMini(); 
                            showToast("⚠️ Tienes un entreno activo.");
                        } catch(e) { console.error("Error parsing saved workout", e); }
                    } else { 
            ...
            console.log("Offline mode or error:", e); 
...
/* ===========================================================
   UI UTILS & NAVIGATION (Mejora 4: Persistencia Nav)
   =========================================================== */
let scrollPos = 0;
...
window.switchTab = (t) => {
    // Si intentamos ir a routines-view pero tenemos un entreno activo
    if (t === 'routines-view' && activeWorkout) {
         // Permitimos ir a routines, pero el botón de "Volver al entreno" debería ser visible o accesible.
         // En esta implementación, simplemente permitimos la navegación. 
         // El estado se guarda en localStorage y variable global.
    }
    
    // Si volvemos a la vista de workout (Mejora 4)
    if (t === 'workout-view') {
        if (!activeWorkout) {
             // Si no hay entreno, redirigir a routines
             t = 'routines-view';
        } else {
             // Si hay entreno, asegurarnos que está renderizado
             renderWorkout();
             startTimerMini();
        }
    }
    ...
    if (t === 'workout-view') {
        // Podríamos iluminar Routines o crear un botón extra, pero por ahora desmarcamos todos
    }
};
...
async function loadRoutines() {
    ...
    onSnapshot(query(collection(db,"routines")), (s)=>{
        ...
        // Botón "Volver al Entreno" si existe (Mejora 4)
        if (activeWorkout) {
             const div = document.createElement('div');
             div.className = 'card';
             div.style.border = '2px solid var(--accent-color)';
             div.style.background = 'rgba(0, 255, 136, 0.1)';
             div.innerHTML = `<h3 style="color:var(--accent-color); text-align:center;">⚡ ENTRENO EN CURSO</h3><button class="btn" onclick="window.switchTab('workout-view')">CONTINUAR</button>`;
             l.appendChild(div);
        }

        if(myRoutines.length === 0) { const msg = document.createElement('div'); msg.style.cssText="padding:20px; text-align:center; color:#666;"; msg.innerText='No tienes rutinas asignadas.'; l.appendChild(msg); return; }
...
/* ===========================================================
   MEJORA 3: RENDER LIMIT & INFINITE SCROLL
   =========================================================== */
window.openEditor = async (id = null) => { 
    ...
    // Inicializar lista para infinite scroll
...
window.filterExercises = (t) => { 
    ...
    initExerciseList(filtered);
};

// Nueva función de inicialización de lista (Mejora 3)
function initExerciseList(list) {
    ...
    filteredExercisesList = sortedList;
    exercisesBatchIndex = 0;
    
    // Crear elemento centinela para el observer
    const sentinel = document.createElement('div');
    sentinel.id = 'exercises-sentinel';
    ...
    exercisesObserver.observe(sentinel);
...
function renderNextBatch() {
    const c = document.getElementById('exercise-selector-list');
    const sentinel = document.getElementById('exercises-sentinel');
    if (!c || !sentinel) return;

    const start = exercisesBatchIndex * BATCH_SIZE;
    const end = start + BATCH_SIZE;
    const batch = filteredExercisesList.slice(start, end);
    ...
    batch.forEach(e => {
        ...
        if (isSelected) {
            ...
            d.onclick = () => { 
                ...
                // Al seleccionar, reiniciamos la lista para que aparezca arriba
                initExerciseList(filteredExercisesList);
                renderSelectedSummary(); 
            };
        }
    ...
    c.insertBefore(fragment, sentinel);
    exercisesBatchIndex++;
}
...
// CACHÉ DIARIO (RESET 00:00)
window.loadRankingView = async () => {
    switchTab('ranking-view'); 
    const list = document.getElementById('ranking-list'); 
    list.innerHTML = '<div style="text-align:center; margin-top:50px; color:#666;">⏳ Verificando datos...</div>';
    ...
    try {
        ...
        if (cachedRaw) {
            ...
            if (cachedObj.date === todayStr) {
                console.log("⚡ USANDO CACHÉ LOCAL (0 Lecturas Firebase)");
                rankingData = cachedObj.data;
                useCache = true;
            }
        }

        if (!useCache) {
            console.log("☁️ DESCARGANDO NUEVO RANKING (Coste Firebase)");
            ...
            if (rankFilterGender !== 'all') { 
                q = query(collection(db, "users"), where("rankingOptIn", "==", true), where("gender", "==", rankFilterGender), orderBy(orderByField, "desc"), limit(50)); 
            }
            ...
            snap.forEach(d => {
                ...
                rankingData.push({
                    ...
                    name: u.name,
                    photo: u.photo,
                    workouts: u.stats?.workouts || 0,
                    value: rawValue
                });
            });

            try {
                const cacheObject = { date: todayStr, data: rankingData };
                localStorage.setItem(cacheKey, JSON.stringify(cacheObject));
            } catch (err) { console.warn("Memoria llena, no se pudo cachear"); }
        }
        ...
        rankingData.forEach(u => {
            ...
            const avatarHtml = u.photo 
                ...
                : `<div class="mini-avatar-placeholder" style="width:35px;height:35px;font-size:0.8rem;">${u.name.charAt(0).toUpperCase()}</div>`;
            ...
            div.innerHTML = `
                ...
                <div style="flex:1;">
                    ...
                    <div style="font-size:0.65rem; color:#666;">${u.workouts} entrenos</div>
                </div>
        ...
        if(useCache) {
            ...
            advice.className = 'tip-box';
            advice.style.fontSize = '0.6rem';
            advice.style.marginTop = '10px';
            advice.style.opacity = '0.7';
            advice.innerHTML = "📊 Datos actualizados de hoy. El ranking se recalculará mañana.";
            list.appendChild(advice);
        }

    } catch(e) { 
        console.error("Rank Error:", e); 
        list.innerHTML = `<div style="text-align:center; color:#666;">Error.<br><small>${e.message}</small></div>`; 
    }
};
...
window.finishWorkout = async (rpeVal) => {
    try {
        ...
        localStorage.removeItem('fit_active_workout'); activeWorkout = null; 
        if (durationInt) clearInterval(durationInt); if (wakeLock) { await wakeLock.release(); wakeLock = null; } window.switchTab('routines-view');
    ...
    try {
        ...
        uSnap.forEach(d => { 
            ...
            if (u.role === 'athlete') { 
                ...
                div.innerHTML = `<input type="checkbox" class="user-mass-check selector-checkbox" value="${d.id}" id="u-${d.id}" style="pointer-events:none;"><label class="selector-label" style="pointer-events:none;">${u.name}</label>`; 
                list.appendChild(div); 
            } 
        });
...
window.viewWorkoutDetails = (wId, routineName, detailsStr, noteStr, timeStr = "") => { 
    try { 
        ...
        let html = `<br>
            ...
            <div id="history-details-container"><br>
                ${renderHistoryHTML(currentHistoryDetails)}<br>
            </div><br>
            <div style="margin-top:20px; text-align:center;"><br>
                <button id="btn-edit-history" class="btn-outline" style="width:auto; border-color:var(--accent-color); color:var(--accent-color);" onclick="window.enableHistoryEdit()">✏️ EDITAR DATOS</button><br>
                <button id="btn-save-history" class="btn hidden" style="width:auto; margin-top:10px;" onclick="window.saveHistoryChanges()">💾 GUARDAR CAMBIOS</button><br>
            </div><br>
        `;
    ...
    items.forEach(item => {
        ...
        item.innerHTML = `<br>
            <div style="display:flex; gap:5px; align-items:center;"><br>
                ...
                <span style="color:#666">x</span><br>
                <input type="number" class="hist-edit-weight" value="${setObj.w}" style="width:40px; padding:2px; margin:0; text-align:center; background:#000; border:1px solid #444;"><br>
            </div><br>
        `;
    });
...
window.openCoachView = async (uid, u) => {
    ...
    if(videoToggleEl) {
         ...
         tgRow.id = 'coach-telegram-row';
         tgRow.style.cssText = "display:flex; justify-content:space-between; align-items:center; margin-top:15px;"; 
         tgRow.innerHTML = `
            <span>Habilitar Chat Telegram</span>
            <label class="switch">
                <input type="checkbox" id="coach-toggle-telegram" onchange="window.toggleUserFeature('allowTelegram', this.checked)">
...
window.toggleUserFeature = async (field, isActive) => {
    ...
    const toggleMap = {
        ...
        'showMeasurements': 'coach-view-measures'
    };
    if(toggleMap[field]) {
        const el = document.getElementById(toggleMap[field]);
        if(el) isActive ? el.classList.remove('hidden') : el.classList.add('hidden');
    }
    if(field === 'showPhotos') {
        const pCard = document.getElementById('coach-view-photos');
        if(pCard) isActive ? pCard.classList.remove('hidden') : pCard.classList.add('hidden');
    }
    try {
        await updateDoc(doc(db, "users", selectedUserCoach), { [field]: isActive });
        console.log(`Updated ${field} to ${isActive}`);
    } catch (e) {
...
window.assignRoutine = async () => {
    ...
    const rid = sel.value;
    if(!rid || !selectedUserCoach) return alert("Selecciona una rutina");
...
window.unassignRoutine = async (rid) => {
    ...
    try {
        ...
        await updateDoc(doc(db, "users", selectedUserCoach), { routineOrder: arrayRemove(rid) });
        alert("🗑️ Rutina retirada.");
        window.openCoachView(selectedUserCoach, selectedUserObj);
    } catch(e) { alert(e.message); }
};
...
window.assignPlan = async () => {
     ...
     const pid = sel.value;
     if(!pid) return;
     if(!confirm("¿Asignar todo este plan?")) return;
     try {
        const snap = await getDoc(doc(db,"plans", pid));
        if(!snap.exists()) return;
        const routines = snap.data().routines || [];
        const promises = routines.map(rid => updateDoc(doc(db,"routines",rid), { assignedTo: arrayUnion(selectedUserCoach) }));
        await Promise.all(promises);
        await updateDoc(doc(db, "users", selectedUserCoach), { routineOrder: routines });
        alert("✅ Plan asignado.");
        window.openCoachView(selectedUserCoach, selectedUserObj);
...
window.approveUser = async () => {
    ...
    if(confirm("¿Aprobar acceso a este atleta?")) {
